...
use language_tutor::{
    KoreanTutorLevel, KoreanTutorMistake, KoreanTutorMistakeKind, ModelConfiguration,
    SpeechSynthesisModel, TextModel, TranscriptionModel,
};
use llm_core::Audio;
...
pub fn router(sessions: Arc<SessionService>) -> Router {
    Router::new()
        ...
        .route(
            "/api/turn-voice-sessions/{id}/audio-turns",
            post(send_audio_turn),
        )
...
async fn create_session(
    ...
    Json(CreateSessionResponse {
        id: sessions
            .create(request.level, request.model_configuration)
            .await,
    })
}
...
impl From<SessionEvent> for ClientEvent {
    fn from(event: SessionEvent) -> Self {
        match event {
            ...
            SessionEvent::TurnMistakes { input_id, mistakes } => {
                Self::TurnMistakes { input_id, mistakes }
            }
    ...
    fn into_response(self) -> Response {
        let (status, message) = match self {
            ...
            Self::Session(SessionError::NotFound) => {
                (StatusCode::NOT_FOUND, "voice session not found".to_owned())
            }
            Self::Session(SessionError::InvalidAudio(message)) => {
